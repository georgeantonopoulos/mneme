...
- **BOTH/CORRECTION path directive ordering** — when a pre-LLM hook injects both a multi-step Mneme protocol AND a path header (e.g. "MNEME BOTH PATH …"), the protocol steps can drown out the user's request. Fix: the injected context MUST begin with a "PRIMARY DIRECTIVE: answer the user's request first" banner that explicitly overrides any "do Mneme writeback first" language. Idempotency guard prevents the banner from being prepended twice on turn-2 re-injection. See `references/hook-directive-order.md` for the template and the bug pattern.
