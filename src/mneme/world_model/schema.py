...
def ensure_world_model_schema(conn: sqlite3.Connection) -> None:
    """Create durable world-model tables without touching graph init."""
    conn.execute(
        """
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wsa_subject ON world_state_assertions(subject_name, predicate, status)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wsa_status ON world_state_assertions(status, state_type)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wsa_source ON world_state_assertions(source_path)")
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wp_due ON world_predictions(status, check_after)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wp_assertion ON world_predictions(subject_assertion_id)")
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_wa_actor ON world_actions(actor, created_at)")
...
def delete_world_model_source(conn: sqlite3.Connection, source_path: str) -> dict[str, int]:
    ...
    The function is intentionally a no-op until the schema exists, so graph-only
    callers do not create world-model tables as a side effect.
    """
